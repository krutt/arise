...
class Igris(Progress):
    ...
    for task in self.tasks:
      if task.fields.get("progress_type") == "build":
        ...
        if task.completed < 0:
          ...
          self.columns = (
            "Preparing to build [magenta_blue]<Image [reset]'[green]"
            f"{ image_name }[reset]'[magenta_blue]>[reset].",
            "".ljust(9),
            BarColumn(),
          )
          ...
          self.columns = (
            f"[blue]Built <[bright_magenta]Image [green]'{image_name}'[reset]>"
  ...
  def progress_build(
    self,
    chunk: Generator[Dict[Literal["stream", "error"], str], None, None],
    task_id: TaskID
  ) -> None:
    for line in chunk:
      if "stream" in line:
